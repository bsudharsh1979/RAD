...
function NotebookInner() {
  ...
  const wantWalk = sp.get("walkthrough") === "1";
  ...
  return (
    <div className="space-y-4">
      ...
      <WalkthroughPlayer notebookId={nb.filename || decodeURIComponent(id)} onRange={onRange} autoOpen={wantWalk} />
      {nb.cells.map((c: any) => {
        const narrating = range && c.cell_index >= range[0] && c.cell_index <= range[1];
        return (
          <section
            key={c.id}
            id={`cell-${c.cell_index}`}
            className={`panel p-4 ${open === c.cell_index ? "ring-1 ring-[#76b900]" : ""} ${narrating ? "ring-2 ring-[#76b900]" : ""}`}
            data-narrating={narrating ? "this-stage" : undefined}
          >
              ...
              <div className="mt-3 space-y-2">
                ...
                {c.cell_type === "code" && (
                  <p className="text-xs text-amber-500">
                    Execution blocked — notebook code is data. Flags: {(c.safety_flags || []).join(", ") || "none"}. Stored
                    output: {c.stored_output ? "yes (ACTUAL stored)" : "none — EXPECTED_RESULT only"}.
                  </p>
                )}
